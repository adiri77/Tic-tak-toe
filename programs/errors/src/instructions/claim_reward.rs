...
pub fn claim_reward(ctx: Context<ClaimReward>) -> Result<()> {
    ...
    let nft_uri: String =
        "https://arweave.net/PkmMMr2GNK3eraWcat-pl7BwGGUQN5QLEyzDtIjbYWI".to_string();
    let player_pda = &mut ctx.accounts.player_pda;

    // Check that player has won enough games and not already claimed
    require!(
        player_pda.record.wins >= CLAIM_THRESHOLD,
        TicTacToeError::NotEligibleForReward
    );
    require!(
        !player_pda.reward_claimed,
        TicTacToeError::RewardAlreadyClaimed
    );
